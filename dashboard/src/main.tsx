...
import type { CaseDetail, CaseRow, ExecutionArtifacts, Overview, PaperPacket, PlannerProfile, PyRITProfile, PyRITQuickstartResult, PyRITWorkbench, ResearchSummary, TaskWorkspace } from "./types";

const drawerWidth = 256;
type Page = "learn" | "home" | "cases" | "experience" | "pyrit" | "research";
...
const statusLabel: Record<string, string> = {
  ...
  undetermined: "未评分", success: "成功", failure: "失败",
};
...
function PyRITLearnPage() {
  const [prompt, setPrompt] = useState("PyRIT 学习样本：验证文本如何经过攻击策略到达目标。");
  const [converter, setConverter] = useState<"raw" | "base64">("raw");
  const [result, setResult] = useState<PyRITQuickstartResult | null>(null);
  const [running, setRunning] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const run = async () => {
    setRunning(true); setError(null);
    try { setResult(await api.runPyritQuickstart(prompt, converter)); }
    catch (err) { setError(err instanceof Error ? err.message : "PyRIT 演示运行失败"); }
    finally { setRunning(false); }
  };
  const stages = [
    ["1", "Objective", "你想测试的目标描述"], ["2", "Attack", "PromptSendingAttack 生成并发送消息"],
    ["3", "Target", "TextTarget 离线接收消息"], ["4", "Result", "AttackResult 记录轮数、结果与元数据"],
  ];
  return <Stack spacing={3.5}>
    <Box sx={{ maxWidth: 830 }}>
      <Typography variant="overline" color="primary.main" fontWeight={700}>先学 PyRIT，再接真实任务</Typography>
      <Typography variant="h3" sx={{ mt: 0.4 }}>只看懂一条最小执行链。</Typography>
      <Typography color="text.secondary" sx={{ mt: 1.1, fontSize: "1.02rem" }}>下面运行的是 PyRIT 原生对象，不访问网络、不调用模型、不读取你的 Cookie。它的唯一目的，是让你亲眼看到一个 Attack 如何把输入交给 Target，并生成 AttackResult。</Typography>
    </Box>
    <Paper variant="outlined" sx={{ p: { xs: 2.25, md: 3 } }}>
      <Grid container spacing={0}>{stages.map(([number, title, detail], index) => <Grid key={title} size={{ xs: 12, sm: 6, lg: 3 }}><Box sx={{ display: "flex", gap: 1.25, pr: { lg: 2 }, pb: { xs: 1.5, lg: 0 }, minHeight: 92 }}><Box sx={{ width: 30, height: 30, borderRadius: "50%", display: "grid", placeItems: "center", bgcolor: index === 3 ? "success.main" : "primary.main", color: "common.white", fontWeight: 700, flexShrink: 0 }}>{number}</Box><Box><Typography fontWeight={700}>{title}</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.25 }}>{detail}</Typography></Box></Box></Grid>)}</Grid>
    </Paper>
    <Grid container spacing={2.5}>
      <Grid size={{ xs: 12, lg: 6 }}><Paper variant="outlined" sx={{ p: { xs: 2.25, md: 3 }, height: "100%" }}>
        <Typography variant="h5">运行一次离线实验</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.6, mb: 2 }}>先保持默认文本。切换 Converter 后，PyRIT 会在发送给 TextTarget 前转换它。</Typography>
        <TextField label="Objective（学习用文本）" value={prompt} onChange={(event) => setPrompt(event.target.value)} multiline minRows={4} fullWidth helperText="这里不是攻击载荷，只是传给 PyRIT 的学习样本。" />
        <Typography variant="subtitle2" sx={{ mt: 2 }}>可选转换器</Typography>
        <ToggleButtonGroup size="small" exclusive value={converter} onChange={(_, value) => value && setConverter(value)} sx={{ mt: 0.75 }}><ToggleButton value="raw">不转换</ToggleButton><ToggleButton value="base64">Base64</ToggleButton></ToggleButtonGroup>
        <Button variant="contained" startIcon={<PlayCircleOutlineRoundedIcon />} disabled={running || !prompt.trim()} onClick={() => void run()} sx={{ mt: 2.5 }}>{running ? "PyRIT 正在运行…" : "运行原生 PyRIT 演示"}</Button>
        {error && <Alert severity="error" sx={{ mt: 2 }}>{error}</Alert>}
      </Paper></Grid>
      <Grid size={{ xs: 12, lg: 6 }}><Paper variant="outlined" sx={{ p: { xs: 2.25, md: 3 }, height: "100%", borderLeft: 4, borderLeftColor: result ? "success.main" : "divider" }}>
        <Typography variant="h5">你会看到的结果</Typography>
        {!result ? <Typography color="text.secondary" sx={{ mt: 1.25, lineHeight: 1.8 }}>点击运行后，这里会显示 PyRIT 实际发送给 Target 的文本，以及原生 AttackResult。因为 TextTarget 不回答、没有配置 Scorer，结果应为 <code>undetermined</code>；这不是失败，而是说明评分器在 PyRIT 中是独立组件。</Typography> : <Stack spacing={1.5} sx={{ mt: 1.5 }}>
          <Box><Typography variant="caption" color="text.secondary">实际发送给 TextTarget 的内容</Typography><Paper variant="outlined" sx={{ p: 1.4, mt: 0.45, bgcolor: "action.hover", overflowWrap: "anywhere" }}><Typography component="code" variant="body2">{result.sent_to_target}</Typography></Paper></Box>
          <Grid container spacing={1.25}><Grid size={6}><Typography variant="caption" color="text.secondary">执行策略</Typography><Typography fontWeight={700}>{result.strategy}</Typography></Grid><Grid size={6}><Typography variant="caption" color="text.secondary">执行轮数</Typography><Typography fontWeight={700}>{result.attack_result.executed_turns}</Typography></Grid><Grid size={6}><Typography variant="caption" color="text.secondary">结果</Typography><StatusChip value={result.attack_result.outcome} /></Grid><Grid size={6}><Typography variant="caption" color="text.secondary">耗时</Typography><Typography fontWeight={700}>{result.attack_result.execution_time_ms} ms</Typography></Grid></Grid>
          <Alert severity="info">{result.attack_result.outcome_reason || "未提供结果原因"}</Alert>
        </Stack>}
      </Paper></Grid>
    </Grid>
    <Paper variant="outlined" sx={{ p: { xs: 2.25, md: 3 } }}><Typography variant="h6">看懂这一步以后，你已经掌握了什么？</Typography><Grid container spacing={2} sx={{ mt: 0.5 }}><Grid size={{ xs: 12, md: 4 }}><Typography fontWeight={700}>Attack 不等于输入</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>攻击策略负责组织和执行；同一策略可以换 Converter、Target 和 Scorer。</Typography></Grid><Grid size={{ xs: 12, md: 4 }}><Typography fontWeight={700}>Target 不等于模型</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>TextTarget 只展示消息；以后可替换为 HTTP、API、网页或本地模型目标。</Typography></Grid><Grid size={{ xs: 12, md: 4 }}><Typography fontWeight={700}>结果不等于通关</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>AttackResult 记录执行事实；真实成功仍应由平台、工具或 UI 证据确认。</Typography></Grid></Grid></Paper>
  </Stack>;
}
...
function TaskWorkspacePage({ workspace, close, refresh }: { workspace: TaskWorkspace; close: () => void; refresh: () => Promise<void> }) {
  ...
  const [campaignTargetKind, setCampaignTargetKind] = useState<"replay" | "pyrit-http">("replay");
  const [replayResponse, setReplayResponse] = useState("本地模拟响应：用于验证记录、预算与状态机。");
  const [pyritProfile, setPyritProfile] = useState<Partial<PyRITProfile>>({
    profile_name: workspace.pyrit_readiness.profile.profile_name ?? "本地 PyRIT HTTP 配置",
    request_reference: workspace.pyrit_readiness.profile.request_reference ?? "",
    placeholder: workspace.pyrit_readiness.profile.placeholder ?? "{PROMPT}",
    response_key: workspace.pyrit_readiness.profile.response_key ?? "",
    prompt_encoding: workspace.pyrit_readiness.profile.prompt_encoding ?? "raw",
    model_name: workspace.pyrit_readiness.profile.model_name ?? "",
    timeout: workspace.pyrit_readiness.profile.timeout ?? 30,
  ...
  const artifactPanel = task.plans[0]?.status === "approved" ? <Paper variant="outlined" sx={{ p: { xs: 2.25, md: 3 }, borderLeft: 4, borderLeftColor: "success.main" }}>
    ...
    {task.campaigns.length > 0 && <Paper variant="outlined" sx={{ p: 1.75, mt: 2 }}><Typography variant="subtitle1" fontWeight={700}>可复现实验导出</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>导出同一份已审核 Campaign，分别交给 Inspect AI 做可重复实验，或交给 Promptfoo 做后续版本回归。模型 Provider 与成功评分规则保持为空，需由你在授权环境中补充。</Typography><Stack spacing={1} sx={{ mt: 1.25 }}>{task.campaigns.map((campaign) => <Stack key={campaign.campaign_id} direction={{ xs: "column", md: "row" }} gap={1} alignItems={{ md: "center" }} justifyContent="space-between"><Typography variant="caption">{campaign.campaign_id} · {campaign.status}</Typography><Stack direction="row" gap={1}><Button size="small" variant="outlined" disabled={saving} onClick={() => void downloadCampaignManifest(campaign.campaign_id, "inspect")}>导出 Inspect</Button><Button size="small" variant="outlined" disabled={saving} onClick={() => void downloadCampaignManifest(campaign.campaign_id, "promptfoo")}>导出 Promptfoo</Button></Stack></Stack>)}</Stack></Paper>}
    <Divider sx={{ my: 2 }} />
    ...
    <Grid container spacing={1.5} sx={{ mt: 0.75, alignItems: "center" }}><Grid size={{ xs: 12, sm: 4 }}><TextField label="最多轮数" type="number" value={maxTurns} onChange={(event) => setMaxTurns(event.target.value)} fullWidth /></Grid><Grid size={{ xs: 12, sm: 4 }}><TextField label="最长时间（秒）" type="number" value={maxSeconds} onChange={(event) => setMaxSeconds(event.target.value)} fullWidth /></Grid><Grid size={{ xs: 12, sm: 4 }}><TextField label="最大成本（USD，留空不限制）" type="number" value={maxCost} onChange={(event) => setMaxCost(event.target.value)} fullWidth /></Grid><Grid size={{ xs: 12 }}><Button variant="contained" disabled={saving} onClick={() => void createPendingCampaign(task.plans[0].plan_id)}>创建待执行 Campaign</Button></Grid></Grid>
    {task.campaigns.some((campaign) => campaign.status === "pending" && campaign.target_kind === "replay") && <Paper variant="outlined" sx={{ p: 1.75, mt: 2, bgcolor: "action.hover" }}><Typography variant="subtitle1" fontWeight={700}>本地 Replay 验证</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>此操作只调用浏览器所在电脑的本地模拟目标，用于验证输入记录、预算扣减与证据链；它不会向外部模型或靶场发送请求。</Typography><TextField label="本地模拟响应" value={replayResponse} onChange={(event) => setReplayResponse(event.target.value)} multiline minRows={2} fullWidth sx={{ mt: 1.25 }} />{task.campaigns.filter((campaign) => campaign.status === "pending" && campaign.target_kind === "replay").map((campaign) => <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" alignItems={{ sm: "center" }} gap={1} sx={{ mt: 1.25 }} key={campaign.campaign_id}><Typography variant="body2">待执行 {campaign.campaign_id} · 0 / {campaign.max_turns ?? "?"} 轮</Typography><Button size="small" variant="outlined" disabled={saving} onClick={() => void runLocalReplay(campaign.campaign_id)}>运行本地 Replay</Button></Stack>)}</Paper>}
    <Paper variant="outlined" sx={{ p: 1.75, mt: 2, borderColor: workspace.pyrit_readiness.ready ? "success.main" : "divider" }}><Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" gap={1}><Box><Typography variant="subtitle1" fontWeight={700}>PyRIT 执行前配置</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>这里只保存非敏感设置和本地文件的引用名称；不会读取请求模板、cookie 或 headers 文件。</Typography></Box><Chip label={workspace.pyrit_readiness.ready ? "执行前检查已就绪" : "仍有检查未完成"} color={workspace.pyrit_readiness.ready ? "success" : "default"} variant={workspace.pyrit_readiness.ready ? "filled" : "outlined"} /></Stack><Grid container spacing={1.25} sx={{ mt: 0.75 }}><Grid size={{ xs: 12, md: 6 }}><TextField label="配置名称" value={pyritProfile.profile_name ?? ""} onChange={(event) => setPyritProfile((current) => ({ ...current, profile_name: event.target.value }))} fullWidth /></Grid><Grid size={{ xs: 12, md: 6 }}><TextField label="请求模板引用（仅名称/备注，不填路径或凭据）" value={pyritProfile.request_reference ?? ""} onChange={(event) => setPyritProfile((current) => ({ ...current, request_reference: event.target.value }))} fullWidth /></Grid><Grid size={{ xs: 12, md: 4 }}><TextField label="输入占位符" value={pyritProfile.placeholder ?? "{PROMPT}"} onChange={(event) => setPyritProfile((current) => ({ ...current, placeholder: event.target.value }))} fullWidth /></Grid><Grid size={{ xs: 12, md: 4 }}><TextField label="响应 JSON 字段（可选）" value={pyritProfile.response_key ?? ""} onChange={(event) => setPyritProfile((current) => ({ ...current, response_key: event.target.value }))} fullWidth /></Grid><Grid size={{ xs: 12, md: 4 }}><TextField label="超时（秒）" type="number" value={pyritProfile.timeout ?? 30} onChange={(event) => setPyritProfile((current) => ({ ...current, timeout: Number(event.target.value) }))} fullWidth /></Grid><Grid size={{ xs: 12 }}><FormControlLabel control={<Checkbox checked={Boolean(pyritProfile.captured_request_reviewed)} onChange={(event) => setPyritProfile((current) => ({ ...current, captured_request_reviewed: event.target.checked }))} />} label="我已在本地确认请求模板含有上述占位符" /><FormControlLabel control={<Checkbox checked={Boolean(pyritProfile.credentials_managed_externally)} onChange={(event) => setPyritProfile((current) => ({ ...current, credentials_managed_externally: event.target.checked }))} />} label="凭据仅保留在外部本地文件中" /></Grid><Grid size={{ xs: 12 }}><Button variant="outlined" disabled={saving} onClick={() => void savePyritProfile()}>保存 PyRIT 配置并重新检查</Button></Grid></Grid><Stack spacing={0.5} sx={{ mt: 1.5 }}>{workspace.pyrit_readiness.checks.map((check) => <Typography variant="caption" key={check.id} color={check.ready ? "success.main" : "text.secondary"}>{check.ready ? "✓" : "○"} {check.label}</Typography>)}</Stack></Paper>
  </Paper> : null;
  return <Stack spacing={3}>
    {artifactPanel}
    {plannerPanel}
...
function App() {
  ...
  const nav = useMemo(() => [
    ["learn", "PyRIT 入门", <SchoolRoundedIcon />], ["home", "完整工作台", <DashboardRoundedIcon />], ["cases", "案例记录", <FolderOpenRoundedIcon />], ["research", "研究分析", <ScienceRoundedIcon />],
  ] as const, []);
  return <ThemeProvider theme={theme}><CssBaseline /><Box sx={{ display: "flex", minHeight: "100vh" }}>
    ...
    <Box component="main" sx={{ flexGrow: 1, p: { xs: 2, md: 4 }, pt: { xs: 11, md: 13 }, maxWidth: 1560, width: "100%" }}>{error && <Alert severity="error" sx={{ mb: 2 }}>{error}。请先运行 <code>python -m redteam_memory serve</code>。</Alert>}{!overview || !research || !paperPacket || !pyritWorkbench ? <Box sx={{ display: "grid", placeItems: "center", minHeight: 320 }}><CircularProgress /></Box> : workspace ? <TaskWorkspacePage workspace={workspace} close={() => setWorkspace(null)} refresh={async () => { await load(); setWorkspace(await api.taskWorkspace(workspace.task.case_id)); }} /> : page === "learn" ? <PyRITLearnPage /> : page === "home" ? <HomePage overview={overview} rows={cases} selectCase={selectCase} goToCases={() => go("cases")} goToExperience={() => go("experience")} openIntake={() => setIntakeOpen(true)} /> : page === "cases" ? <CasesPage rows={cases} selectCase={selectCase} /> : page === "experience" ? <ExperiencePage rows={cases} goToCases={() => go("cases")} /> : page === "pyrit" ? <PyRITWorkbenchPage workbench={pyritWorkbench} openTask={selectCase} /> : <ResearchPage summary={research} packet={paperPacket} />}</Box>
